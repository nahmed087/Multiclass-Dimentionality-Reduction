...
%% Make a Small GUI with buttons
button = menu('Select an Image', 'Speed Limit 20', 'Speed Limit 60', 'STOP', 'No Entry', 'Turn Right');
if button == 1
	baseFileName = 'sign20.ppm';
elseif button == 2
	baseFileName = 'sign60.ppm';
elseif button == 3
	baseFileName = 'signstop.ppm';
elseif button == 4
	baseFileName = 'signnoentry.ppm';
elseif button == 5
	baseFileName = 'signturnright.ppm';
end
...
[coeff,score,latent,tsquared,explained,mu] = pca(listOfRGBValues);
...
%% Plot other relevant plots
PCArdplot(baseFileName,score,coeff)
